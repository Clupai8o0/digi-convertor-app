...
const TypeReverseButton = () => {
	...
	function getPathColor(from?: Boolean) {
		let type = null;
		if (from) {
			type = fromType;
		} else type = toType;
		...
		if (type === Types.Binary) {
			return styles.binary;
		} else if (type === Types.Decimal) {
			return styles.decimal;
		} else if (type === Types.Octal) {
			return styles.octal;
		} else {
			return styles.hexadecimal;
		}
	}

	return (
		<button
			onClick={handleTypeReversal}
			className={styles.btnReverse}
			title="Reverse Button"
			style={{
				backgroundImage: `linear-gradient(rgba(255, 255, 255, 0),rgba(255, 255, 255, 0)),linear-gradient(180deg, var(--${fromType.toLowerCase()}), var(--${toType.toLowerCase()}))`,
			}}
		>
			<svg
				width="20"
